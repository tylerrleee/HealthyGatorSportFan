...
The feasibility thresholds are informed by synthetic sensitivity analysis calibrated for the 14-day study protocol (5 EMA prompts per participant per day; total expected N \= 70 prompts).

# **Feasibility Definitions & Criteria**

* 1\. EMA Response & Check-in Completion  
  * **Completed Check-in Definition:** An EMA is counted as completed when the scheduled prompt is delivered via the app and the participant successfully submits responses to **all** required items on screen within the 60-minute assigned response window. Missing required values cause the prompt to be treated as unanswered.  
  * **Prompt Response (Partial) Definition:** A prompt is considered "responded" if the participant submits at least one item response within the 60-minute window, even if required items remain unanswered.  
  * **Calculations:**  
    * **Numerator (Completed Check-ins):** Number of EMA prompts with all required items submitted within 60 minutes.  
    * **Numerator (Prompt Responses):** Number of EMA prompts with \&ge; 1 item submitted within 60 minutes.  
    * **Denominator:** Total EMA prompts successfully delivered during the 14-day study. Prompts with documented push-notification or app-delivery failures are excluded from the denominator (reported separately).  
  * **Feasibility Benchmark vs. Analytic Requirement:**  
    * **Preregistered Feasibility Benchmark:** **75%** response rate across the 14-day protocol.  
...
* 3\. Participant Retention  
  * **Definition:** A participant is retained if they remain enrolled through the full 14-day study period without dropping out. Missing EMA prompts does not constitute a dropout unless the participant formally withdraws or is withdrawn.  
  * **Calculations:**  
    * **Numerator:** Number of participants enrolled continuously from Day 1 through Day 14\.  
...
* **Dashboard vs. Offline Analysis:**  
  ...
  * **Offline:** MSSD calculations, AR(1) parameter recovery (threshold \<80%), response quality, compliance regressions.

## **JITAI Check-ins and Intervention Log**

* **Source Tables:** jitai\_log, ema, engagement\_log  
* **Metrics:** Intervention dosage (3–7 prompts/week), push delivery funnel latency, decision audit logs, cooldown compliance (min 60-min trigger gap).  
* **Dashboard vs. Offline Analysis:**  
